...
Spec nó phải theo: `DE-BAI-VONG12.md`. Báo cáo của nó: `BAO-CAO-VONG12.md`.
...
## Định dạng
- Ghi `DAP-VONG12.md`, tiếng Việt, **≤70 dòng**, bảng phẳng, có emoji.
...
---
## ⚠️ VÒNG 10 — phạm vi
Ba lỗi vòng 9 đã giao vá. Nặng nhất: maker **cắt `--dry-run`** khỏi `commands/failover.md` → `/polykit:failover` **gửi Telegram thật**, và **báo cáo im lặng**.

Kiểm theo thứ tự:
1. 🔴 `grep -n "dry-run" commands/failover.md` → **phải có**. Notifier giả: plugin-style → `notified: false`; CLI trần → `notified: true`.
2. 🔴 `grep -rn "send" README.md commands/ CLAUDE.md` → hết mâu thuẫn. `--send` không tồn tại trong CLI.
3. 🔴 Test "không nhận cờ" phải đi **đường CLI thật** + đối tượng **thật sự** không nhận cờ. **Tự bẻ hành vi để thử**: bẻ → test phải ĐỎ. Nếu bẻ mà vẫn xanh, test vẫn rỗng ruột, báo dòng đầu.
4. `claude` **cả 4 nhánh** → `served_model=claude-opus-5`.
5. Test ≥152, 0 failed. 7/7 `--dump-config` exit 0.
6. 🔍 **Mục "HÀNH VI ĐÃ ĐỔI"**: báo cáo maker có mục đó chưa? Có hành vi nào bị đổi mà **không** khai? Đây là điểm soi chính lượt này — maker có trait **reward hacking**, đã bẻ hành vi 2 vòng liền để làm test/tài liệu khớp.
7. Lỗi **MỚI**.

---
## 🧊 VÒNG 11 — VÒNG CUỐI, PHẠM VI ĐÓNG BĂNG
Maker chỉ được làm **ĐÚNG 2 VIỆC**: (1) dựng **vendor giả cố định trong fixture** rồi chuyển 3 test về dùng nó; (2) nhánh `not_installed` **thôi bịa slug**.

Kiểm ĐÚNG những điều sau, **đừng mở rộng**:
1. 🔴 **Phép revert từng test**: bẻ hành vi → test phải **ĐỎ**. Đặc biệt: inject thêm một dòng warning → `test_vong7_text_duplicate_warning` phải ĐỎ (vòng 10 bạn bẻ mà nó vẫn xanh).
2. 🔴 Vendor giả `model_flag: null` → `served_model=null` + warning ở **CẢ 4 nhánh** (ok/error/quota/not_installed). Vendor giả **có** cờ → slug đúng ở cả 4.
3. 🔴 Vendor giả **KHÔNG được** vào `config/vendors.json`: `git diff --quiet config/vendors.json; echo $?` → `0`.
4. 🔴 `grep -n "dry-run" commands/failover.md` → phải còn.
5. Test ≥152, 0 failed. 7/7 `--dump-config` exit 0.
6. 🧊 **Maker có làm gì NGOÀI 2 việc không?** `git diff --stat` — file nào bị sửa mà không thuộc phạm vi thì báo. Đây là điểm soi chính lượt này.
7. Báo cáo có mục **"HÀNH VI ĐÃ ĐỔI"** và **"PHÁT HIỆN THÊM — CHƯA VÁ"** chưa?

---
## 🧊 VÒNG 12 — VÒNG CUỐI, ĐÚNG 1 VIỆC
Maker chỉ được **thêm test khoá nhánh `not_installed`**. Hành vi đã đúng từ vòng 11, chỉ thiếu test canh.

Kiểm ĐÚNG những điều sau:
1. 🔴 **Phép revert**: bẻ nhánh skip như bạn đã bẻ vòng 11 (`served_model = None if model=="auto" else model`, bỏ `finalize`) → test mới phải **ĐỎ**. Bẻ mà vẫn xanh = test vô nghĩa, báo dòng đầu.
2. 🔴 Test **>152**, 0 failed.
3. 🔴 `git diff --stat` → **chỉ sửa file test**. Sửa file khác = phá phạm vi đóng băng, báo ngay.
4. 🔴 `git diff --quiet config/vendors.json; echo $?` → `0` · `grep -n "dry-run" commands/failover.md` → còn · 7/7 `--dump-config` exit 0.
5. 🚨 Báo cáo có còn khai lệnh `python` trần không (lần 7)? Cổng tự động đã grep, nhưng bạn xác nhận lại.
6. Lỗi **MỚI** — nếu không có, ghi `KHÔNG TÌM RA CHỖ HỎNG`.
