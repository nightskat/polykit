...
S=TRANG-THAI-VONG6.md; P=~/.pyenv/versions/3.11.8/bin/python
...
for i in 1 2 3; do
  ...
  agy --model gemini-3.1-pro-high --print-timeout 25m --dangerously-skip-permissions \
      --print "Đọc file ~/Developer/polykit/DE-BAI-VONG6.md và thực hiện đúng toàn bộ yêu cầu trong đó. GHI FILE BAO-CAO-VONG6.md ngay sau khi vá xong lỗi đầu tiên." > .m6.out 2> .m6.err
  n=$([ -f BAO-CAO-VONG6.md ] && wc -l < BAO-CAO-VONG6.md || echo 0)
  echo "- exit=$? · out=$(wc -c < .m6.out) byte · báo cáo=$n dòng · $(date '+%H:%M')" >> $S
...
grok --permission-mode bypassPermissions --prompt-file ./DE-BAI-DAP-VONG6.md > .q6.out 2> .q6.err
echo "- exit=$? · báo cáo=$([ -f DAP-VONG6.md ] && wc -l < DAP-VONG6.md || echo 0) dòng · $(date '+%H:%M')" >> $S
