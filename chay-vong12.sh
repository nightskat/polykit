...
S=TRANG-THAI-VONG12.md; P=~/.pyenv/versions/3.11.8/bin/python
...
for i in 1 2 3; do
  ...
  agy --model gemini-3.1-pro-high --print-timeout 25m --dangerously-skip-permissions \
      --print "Đọc file ~/Developer/polykit/DE-BAI-VONG12.md và thực hiện đúng toàn bộ yêu cầu trong đó. GHI FILE BAO-CAO-VONG12.md ngay sau khi vá xong lỗi đầu tiên." > .m12.out 2> .m12.err
  n=$([ -f BAO-CAO-VONG12.md ] && wc -l < BAO-CAO-VONG12.md || echo 0)
  echo "- exit=$? · out=$(wc -c < .m12.out) byte · báo cáo=$n dòng · $(date '+%H:%M')" >> $S
  [ "$n" -ge 5 ] && { echo "- ✅ có báo cáo" >> $S; break; }
  echo "- ⚠️ chưa ra báo cáo, thử lại" >> $S
done
echo "## Cổng chặn — không do AI làm" >> $S
if [ -f BAO-CAO-VONG12.md ] && grep -qE '(^|[^3a-zA-Z._/-])python ' BAO-CAO-VONG12.md; then
  echo "- 🔴 BÁO CÁO KHAI LỆNH \`python\` TRẦN (không chạy được, exit 127) -> TỪ CHỐI" >> $S
  grep -nE '(^|[^3a-zA-Z._/-])python ' BAO-CAO-VONG12.md | head -3 >> $S
else
  echo "- ✅ báo cáo không khai lệnh \`python\` trần" >> $S
fi
...
grok --permission-mode bypassPermissions --prompt-file ./DE-BAI-DAP-VONG12.md > .q12.out 2> .q12.err
echo "- exit=$? · báo cáo=$([ -f DAP-VONG12.md ] && wc -l < DAP-VONG12.md || echo 0) dòng · $(date '+%H:%M')" >> $S
