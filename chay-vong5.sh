...
S=TRANG-THAI-VONG5.md; P=~/.pyenv/versions/3.11.8/bin/python
...
for i in 1 2 3; do
  ...
  agy --model gemini-3.1-pro-high --print-timeout 25m --dangerously-skip-permissions \
      --print "Đọc file ~/Developer/polykit/DE-BAI-VONG5.md và thực hiện đúng toàn bộ yêu cầu trong đó. GHI FILE BAO-CAO-VONG5.md ngay sau khi vá xong lỗi đầu tiên." > .m5.out 2> .m5.err
  n=$([ -f BAO-CAO-VONG5.md ] && wc -l < BAO-CAO-VONG5.md || echo 0)
  echo "- exit=$? · out=$(wc -c < .m5.out) byte · báo cáo=$n dòng · $(date '+%H:%M')" >> $S
...
grok --permission-mode bypassPermissions --prompt-file ./DE-BAI-DAP-VONG5.md > .q5.out 2> .q5.err
echo "- exit=$? · báo cáo=$([ -f DAP-VONG5.md ] && wc -l < DAP-VONG5.md || echo 0) dòng · $(date '+%H:%M')" >> $S
