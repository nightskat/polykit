...
S=TRANG-THAI-VONG10.md; P=~/.pyenv/versions/3.11.8/bin/python
...
for i in 1 2 3; do
  ...
  agy --model gemini-3.1-pro-high --print-timeout 25m --dangerously-skip-permissions \
      --print "Đọc file ~/Developer/polykit/DE-BAI-VONG10.md và thực hiện đúng toàn bộ yêu cầu trong đó. GHI FILE BAO-CAO-VONG10.md ngay sau khi vá xong lỗi đầu tiên." > .m10.out 2> .m10.err
  n=$([ -f BAO-CAO-VONG10.md ] && wc -l < BAO-CAO-VONG10.md || echo 0)
  echo "- exit=$? · out=$(wc -c < .m10.out) byte · báo cáo=$n dòng · $(date '+%H:%M')" >> $S
...
grok --permission-mode bypassPermissions --prompt-file ./DE-BAI-DAP-VONG10.md > .q10.out 2> .q10.err
echo "- exit=$? · báo cáo=$([ -f DAP-VONG10.md ] && wc -l < DAP-VONG10.md || echo 0) dòng · $(date '+%H:%M')" >> $S
