...
S=TRANG-THAI-VONG7.md; P=~/.pyenv/versions/3.11.8/bin/python
...
for i in 1 2 3; do
  ...
  agy --model gemini-3.1-pro-high --print-timeout 25m --dangerously-skip-permissions \
      --print "Đọc file ~/Developer/polykit/DE-BAI-VONG7.md và thực hiện đúng toàn bộ yêu cầu trong đó. GHI FILE BAO-CAO-VONG7.md ngay sau khi vá xong lỗi đầu tiên." > .m7.out 2> .m7.err
  n=$([ -f BAO-CAO-VONG7.md ] && wc -l < BAO-CAO-VONG7.md || echo 0)
  echo "- exit=$? · out=$(wc -c < .m7.out) byte · báo cáo=$n dòng · $(date '+%H:%M')" >> $S
...
grok --permission-mode bypassPermissions --prompt-file ./DE-BAI-DAP-VONG7.md > .q7.out 2> .q7.err
echo "- exit=$? · báo cáo=$([ -f DAP-VONG7.md ] && wc -l < DAP-VONG7.md || echo 0) dòng · $(date '+%H:%M')" >> $S
